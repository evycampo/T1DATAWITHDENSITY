...
```{r}
HighMH = ifelse(clean_heart$MentalHealth>=15, "No", "Yes")
LowPHA = ifelse(clean_heart$PhysicalActivity>=15, "No", "Yes")
clean_heart = data.frame(clean_heart, High, LowPHA)
```
...
```{r, include=T}
loadPkg("caret") 
cm = confusionMatrix( predict(tree1, type = "class"), reference = tree1[, "tree1"] )
print('Overall: ')
cm$overall
print('Class: ')
cm$byClass
