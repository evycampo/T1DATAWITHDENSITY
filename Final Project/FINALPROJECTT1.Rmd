...
The first step is to some pre-processing work. First, because I will use *bestglm::bestglm()*, a feature selection method, to decide which variables are important and which are not, I must clean dataframe with target variable renamed as y, and all other
unused variables removed from dataframe. So I put the HeartDisease column in the end of the dataset and rename it into y.

Second, considering that there are too little rows with the value "Yes (during pregnancy)" in Diabetic variable, I combine the value "Yes (during pregnancy)" and "Yes" together in Diabetic variable.

These are the two pre-processing steps in model building part.
...
After preprocessing, I need to balance the data. To get a look of the proportion of heart disease data before we continue our research.

```{r results='markup'}
table( heartdata2$y )
```
We can find that the dataset is very unbalanced, Only 8.6% of the dataset have value of 1 for y (HeartDisease). Considering that the dataset is large, so I use undersampling methods to balance the dataset. After the balancing work, the value zero and value one of y are the same. And there is the reference for different balancing method:  https://www.analyticsvidhya.com/blog/2016/03/practical-guide-deal-imbalanced-classification-problems/
...
Before we begin logistic regression model, let's have a look of the structure of the dataset now.
```{r results='markup'}
...
To train and evaluate the model later, I split the dataset into two parts. 80 percent of the dataset will be used to train the model and the rest 20% will be used to test the accuracy of the model. I will use *createDataPartition* in the *caret* library to split the dataset.
```{r}
loadPkg("caret")
set.seed(4321)
...
We can find from the model that the p-value of Race and PhysicalActivity are larger than 0.05, which means these two variables are insignificant. So just drop these two variables and make the second logistic regression model again.
...
We can find that all the p-values of the model indicate significance, meaning that our model is a legitimate one. And R square of 0.29 tells that 29 percent of the variance is explained.
...
After we finish this, we can have a look of the Variance Inflation Factor (vif).
...
We can find that some vif values are larger than 10, which means these variables are highly correlated, not acceptable. So I tried to drop one variable a time. At mean time, look at the p-value to ensure that the variables are significant. At the end, I got the model like below:
...
## Feature selection
In this part, I want to use feature selection to find out the most suitable variables from our current model. Because the data_train dataset which has more than 40,000 rows is large and takes a lot of time to run. I changed to data_test to do the feature selection. Data_test has the same data structure but less rows.
